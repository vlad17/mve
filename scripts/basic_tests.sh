...
main() {
    ...
    flags+=("bootstrap $common_flags $mpc_flags $con_flags")
    flags+=("bootstrap $common_flags $mpc_flags $con_flags --explore_std 1")
    flags+=("dagger $common_flags $mpc_flags $con_flags")
    flags+=("dagger $common_flags $mpc_flags $con_flags --delay 5")
    flags+=("random $common_flags --env_name hc-easy")
    flags+=("bootstrap $common_flags $mpc_flags $con_flags --deterministic_learner")
    flags+=("bootstrap $common_flags $mpc_flags $con_flags --no_extra_explore")
