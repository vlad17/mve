    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if true_dynamics: # learnt dynamics not yet supported
            ...
            for i in range(horizon):
                ...
                state, reward, done = tf.split(tf.reshape(tf.py_func(sim.simulate, [state, action], tf.float32), [-1, 6]), [4, 1, 1], 1)
            ...
            if double_q:
                ...
                q_tp1_best = total_reward + (1.0 - max_done) * (gamma**(horizon + 1)) * tf.reduce_sum(q_func(state, num_actions, scope="target_q_func", reuse=True) * tf.one_hot(q_tp1_best_using_online_net, num_actions), 1)
            else:
                q_tp1_best = total_reward + (1.0 - max_done) * (gamma**(horizon + 1)) * tf.reduce_max(q_func(state, num_actions, scope="target_q_func", reuse=True), 1)
        q_tp1_best_masked = (1.0 - done_mask_ph) * q_tp1_best

        # compute RHS of bellman equation
        q_t_selected_target = rew_t_ph + q_tp1_best_masked
